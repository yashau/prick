<script lang="ts">
	...
	/*
	 * `ref` is narrowed away from the registry default of `HTMLElement | null`.
	 *
	 * `worker-configuration.d.ts` (generated by `wrangler types`) declares a
	 * global `interface Element` for HTMLRewriter, which MERGES with the DOM's
	 * `Element` and adds a `remove(): Element` overload to it. `HTMLSelectElement`
	 * declares its own `remove()` overloads, so it no longer satisfies the merged
	 * `HTMLElement` and `bind:this` on the `<select>` below stops typechecking.
	 *
	 * This is the only registry component the collision reaches: `select` and
	 * `option` are the two DOM interfaces that redeclare `remove()`. Naming the
	 * concrete element type here is both more accurate than `HTMLElement` and
	 * side-steps the merge entirely.
	 */
	type NativeSelectProps = Omit<WithElementRef<HTMLSelectAttributes>, "size" | "ref"> & {
		size?: "sm" | "default";
